...
public class CameraSource {
  ...
  private final HashMap<Integer, VisionImageProcessor> frameProcessors = new HashMap<>();

  private KETIDetector ketiDetector;

  // FPS 측정을 위한 변수들
  private long startTime = 0;
  private int frameCount = 0;

  public void setModeOnOff(int type)
  {
      ketiDetector.setMode(type);

      VisionImageProcessor processor = frameProcessors.get(KETIDetectorConstants.POSE_DETECTOR_PROCESSOR);
      PoseDetectorProcessor poseDetectorProcessor = (PoseDetectorProcessor)processor;
      poseDetectorProcessor.setPoseType(type);
  ...
  public CameraSource(Activity activity, GraphicOverlay overlay, Handler handler) {
    this.activity = activity;
    graphicOverlay = overlay;
    graphicOverlay.clear();
    processingRunnable = new FrameProcessingRunnable();
    ketiDetector = new KETIDetector(handler);
  }
  ...
  public void addMachineLearningFrameProcessor(VisionImageProcessor processor, int id){
    synchronized (processorLock) {
      cleanScreen();
      frameProcessors.put(id, processor);
      //frameProcessors.add(processor);
    }
  }

  public void initFrameProcessor(){
    ketiDetector.initializeProcessors(frameProcessors);
  }
  ...
  private class FrameProcessingRunnable implements Runnable {
    ...
    void setNextFrame(byte[] data, Camera camera) {
      synchronized (lock) {
        ...
        //검출 결과 수집
        ketiDetector.collectResult(frameProcessors);
        Log.d(TAG, "  ->"+(System.currentTimeMillis()/1000.0));
    ...
    public void run() {
      ...
      while (true) {
        ...
        try {
          synchronized (processorLock){
            int size = frameProcessors.size();
            /*
            VisionImageProcessor action_processor = frameProcessors.get(KETIDetectorConstants.ACTION_DETECTOR_PROCESSOR);
            if(action_processor != null)
            {
              action_processor.processByteBuffer(
                      data,
                      new FrameMetadata.Builder()
                              ...
                              .build(),
                      graphicOverlay,
                      KETIDetectorConstants.ACTION_DETECTOR_PROCESSOR);
            }
            */
            VisionImageProcessor face_processor = frameProcessors.get(KETIDetectorConstants.FACE_DETECTOR_PROCESSOR);
            if(face_processor != null)
            {
              face_processor.processByteBuffer(
                      data,
                      new FrameMetadata.Builder()
                              .setWidth(previewSize.getWidth())
                              .setHeight(previewSize.getHeight())
                              ...
                              .build(),
                      graphicOverlay,
                      KETIDetectorConstants.FACE_DETECTOR_PROCESSOR);
            }


            VisionImageProcessor pose_processor = frameProcessors.get(KETIDetectorConstants.POSE_DETECTOR_PROCESSOR);
            if(pose_processor != null)
            {
              pose_processor.processByteBuffer(
                      data,
                      new FrameMetadata.Builder()
                              .setWidth(previewSize.getWidth())
                              .setHeight(previewSize.getHeight())
                              ...
                              .build(),
                      graphicOverlay,
                      KETIDetectorConstants.POSE_DETECTOR_PROCESSOR);
            }

            VisionImageProcessor action_processor_tmp = frameProcessors.get(KETIDetectorConstants.ACTION_DETECTOR_PROCESSOR_TMP);
            if(action_processor_tmp != null)
            {
              action_processor_tmp.processByteBuffer(
                      data,
                      new FrameMetadata.Builder()
                              .setWidth(previewSize.getWidth())
                              .setHeight(previewSize.getHeight())
                              ...
                              .build(),
                      graphicOverlay,
                      KETIDetectorConstants.ACTION_DETECTOR_PROCESSOR_TMP);
            }

            /*
            for (int i = 0; i < size; i++) {
              VisionImageProcessor processor = frameProcessors.get(i);
              Log.v(TAG, processor.getClass().getName());

              if (i == 0) {
                processor.processByteBuffer(
                        data,
                        new FrameMetadata.Builder()
                                .setWidth(previewSize.getWidth())
                                .setHeight(previewSize.getHeight())
